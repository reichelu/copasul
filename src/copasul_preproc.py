...
def pp_main(copa, f_log_in=''):
    ...
    for ii in range(len(ff['f0'])):
        ...
        for i in range(opt['fsys']['nc']):
            ...
            myLog("\tfile {}, channel {}".format(utils.stm(ff['f0'][ii]), i+1))
...
def pp_point2segment(pnt, pnt_ut, pnt_lab, chunk, chunk_ut, opt):  # !g
    ...
    # output
    seg, seg_ut, seg_lab = utils.ea(), utils.ea(), []
    ...
    for i in utils.idx_a(len(pnt)):
        ...
        # update output
        seg = utils.push(seg, [t_on, t_off])
        seg_ut = utils.push(seg_ut, [t_on_ut, t_off_ut])
...
def pp_loc_w_nrm(loc, glb, opt):
    ...
    return utils.cellwise(utils.trunc2, tn)
...
def too_short(typ, seg, fstm):
    ...
    if ((seg[1] <= seg[0]) or
        ...
        myLog("WARNING! {}: {} segment too short: {} {}. Segment skipped.".format(
            fstm, typ, seg[0], seg[1]))
        return True
    return False
...
def pp_loc_merge(ag, lab_ag, acc, lab_acc, opt):
    ...
    for i in range(len(ag)):
        ...
        if len(acc[jj]) == 0:
            ...
            a = acc[jj][0]
        d = utils.push(d, [ag[i, 0], ag[i, 1], a])
        lag.append(lab_ag[i])
        lacc.append(lab_acc[jj])

    return utils.cellwise(utils.trunc2, d), d, lag, lacc
...
def pp_grp(copa, ii, i):
    ...
    if len(opt['lab']) > 0:
        ...
        for j in utils.idx_a(len(g)):
            if j >= len(opt['lab']):
                myLog("ERROR! {} cannot be split into grouping values".format(
                    myStm), True)
            lab = opt['lab'][j]
            if len(lab) == 0:
                continue
    ...
    for rt in pp_tiernames(opt['fsys'], ft, 'tier_rate', i, tn_opt):
        ...
        if rt not in tg['item_name']:
            ...
            if 'sloppy' in opt:
                ...
                myLog("ERROR! Annotation file does not (yet) contain tier {} which is required by the tier_rate element for feature set {}.".format(
                    rt, ft), True)
    ...
    for rt in rts:
        ...
        if rt not in tg['item_name']:
            ...
            if crt not in tg['item_name']:
                myLog(
                    "WARNING! Tier {} does not exist. Cannot determine event rates for this tier.".format(rt))
                continue
            else:
                x = crt
        tt[x] = utils.ea()
        ...
        if 'intervals' in t:
            for i in utils.numkeys(t['intervals']):
                if pp_is_pau(t['intervals'][i]['text'], lab_pau):
                    continue
                tt[x] = utils.push(tt[x], np.asarray(
                    [t['intervals'][i]['xmin'], t['intervals'][i]['xmax']]))
        elif 'points' in t:
            for i in utils.numkeys(t['points']):
                tt[x] = utils.push(tt[x], t['points'][i]['time'])
        tt[x] = utils.cellwise(utils.trunc2, tt[x])
    return tt
    ...
    if len(ff['annot']) == 0:
        ...
        for i in range(len(gg)):
            f = os.path.join(opt['fsys']['annot']['dir'],
                             "{}.{}".format(utils.stm(gg[i]), opt['fsys']['annot']['ext']))
            ff['annot'].append(f)
...
def pp_t2i(b, typ, opt, t_chunks, untrunc=False):
        ...
        if nc == 1:
            ...
            for i in range(len(b)):
                ...
                if (ci1 == ci2 or ci2 < 0 or opt['styl']['bnd']['cross_chunk']):
                    bb = utils.push(bb, [o, c])
                # different chunks -> onset is chunk onset of current time stamp
                else:
                    bb = utils.push(bb, [t_chunks[ci2, 0], c])
                # nrm window
                ww = pp_limit_win(c, wl_nrm, t_chunks[ci2, :])
                bb_nrm = utils.push(bb_nrm, [ww[0], c, ww[1]])
                # trend window
                bb_trend = utils.push(
                    bb_trend, [t_chunks[ci2, 0], c, t_chunks[ci3, 1]])
                # update onset
                o = c
            ...
            if ci < 0:
                ci = len(t_chunks)-1
            if o < t_chunks[ci, 1]:
                bb = utils.push(bb, [o, t_chunks[ci, 1]])
        else:
            ...
            for i in range(len(b)):
                ...
                if c2 != c:
                    ...
                    bb_nrm = utils.push(bb_nrm, [ww[0], c, c2, vv[1]])
                else:
                    bb_nrm = utils.push(bb_nrm, [ww[0], c, c2, ww[1]])
                # trend window
                bb_trend = utils.push(bb_trend, [r[0], c, c2t, r2t])
    ...
    # gnl, rhy
        ...
        for i in range(len(b)):
            ...
            if nc == 1:
                # analysis window
                bb = utils.push(bb, pp_limit_win(c, wl, r))
            # nrm interval
            ...
            off = max([bb[i, 1], oo[1]])
            # nrm window
            bb_nrm = utils.push(bb_nrm, [on, off])
            # trend window
            bb_trend = utils.push(bb_trend, [r[0], c, r[1]])

    if untrunc == False:
        bb = utils.cellwise(utils.trunc2, bb)
        bb_nrm = utils.cellwise(utils.trunc2, bb_nrm)
        bb_trend = utils.cellwise(utils.trunc2, bb_trend)
...
def pp_tier_class(tn, annot, typ):
    ...
    Returns: 'segment'|'event'|'' (TextGrid types 'intervals','points' matched segment/event)
    '''
...
def pp_read(an, opt, tn='', fn='', call=''):
    ...
    # xml input
        ...
        # 'segment' or 'event'
        tt = t['type']
        for i in utils.numkeys(t['items']):
            lab.append(t['items'][i]['label'])
            if tt == 'segment':
                d = utils.push(d, [float(t['items'][i]['t_start']),
                                   float(t['items'][i]['t_end'])])
            else:
                d = utils.push(d, float(t['items'][i]['t']))
    ...
    # TextGrid input
        ...
        for i in utils.numkeys(t[kk]):
            ...
            if kk == 'intervals':
                d = utils.push(d, [float(t[kk][i]['xmin']),
                                   float(t[kk][i]['xmax'])])
            else:
                d = utils.push(d, [float(t[kk][i]['time'])])
    ...
    return utils.cellwise(utils.trunc2, d), d, lab
...
def pp_t_uniq(x):
    ...
    x = np.concatenate((utils.lol(t).T, utils.lol(x[i, 1]).T), axis=1)
    ...
    d = np.asarray(list(d))
    z = np.zeros(len(d))
    add = np.concatenate((utils.lol(d).T, utils.lol(z).T), axis=1)
...
def pp_loc(x, opt):
    ...
    return utils.cellwise(utils.trunc2, x)
    ...
    for x in ['f0', 'glob', 'loc', 'bnd', 'gnl_f0', 'gnl_en', 'rhy_f0', 'rhy_en']:
        ...
        for y in ['dir', 'ext', 'typ']:
            if y not in opt['fsys'][x]:
                h_log.write(
                    "ERROR! config.fsys.{} does not contain {} field.\n".format(x, y))
                ec = 2
                continue
...
def diagnosis_seg(c, ii, i, dom, ec, h_log):
    ...
    # min seg length
    min_l = 0.03
    f = "{}/{}.{}".format(c[ii][i]['fsys'][dom]['dir'], c[ii]
                          [i]['fsys'][dom]['stm'], c[ii][i]['fsys'][dom]['ext'])
    for j in utils.numkeys(c[ii][i][dom]):
        ...
        if c[ii][i][dom][j]['t'][1]-c[ii][i][dom][j]['t'][0] < min_l:
            h_log.write("ERROR! {}:interval {} {}:{} segment too short!\n".format(
                f, c[ii][i][dom][j]['to'][0], c[ii][i][dom][j]['to'][1], dom))
            ec = 2
        ...
        for j in utils.numkeys(c[ii][i][dom]):
            if ((c[ii][i][dom][j]['t'][2] <= c[ii][i][dom][j]['t'][0]) or
                ...
                h_log.write("WARNING! {}:interval {} {}:local segments center not within its intervals. Set to midpoint\n",
                            f, c[ii][i][dom][j]['to'][0], c[ii][i][dom][j]['to'][1])
                c[ii][i][dom][j]['t'][2] = utils.trunc2(
                    (c[ii][i][dom][j]['t'][0]+c[ii][i][dom][j]['t'][1])/2)
