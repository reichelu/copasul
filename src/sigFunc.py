...
def dct_fbin(f, c, opt):
    ...
    for j in utils.idx_a(len(fb)-1):
        k = utils.intersect(utils.find(f, '>=', fb[j]),
                            utils.find(f, '<=', fb[j+1]))
        cbin[j] = sum(abs(c[k]))
...
def dct_trunc(f, ci, opt):
    ...
    if opt['lb'] > 0:
        ihp = utils.find(f, '>=', opt['lb'])
    else:
        ihp = ci
    if opt['ub'] > 0:
        ilp = utils.find(f, '<=', opt['ub'])
    else:
        ilp = ci
...
def pau_detector_red(t, e_ratio, opt):
    ...
    while len(t) > n:
        i = utils.find(e_ratio, 'is', 'max')
        j = utils.find(np.arange(1, len(e_ratio), 1), '!=', i[0])
        t = t[j,]
        e_ratio = e_ratio[j]
...
def syl_ncl(s, opt={}):
    ...
    if len(t) > 1:
        for i in range(len(ti)-1):
            ...
            j_min = utils.find(all_e[j], 'is', 'min')
            if len(j_min) == 0:
                j_min = [0]
            # bnd idx
            bj = j[0]+j_min[0]
